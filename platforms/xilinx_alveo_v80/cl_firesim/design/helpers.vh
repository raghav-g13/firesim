...
// DDR4 port definition for the V80: single-channel, 72-bit data w ECC, 18-bit DQS 
// parity enabled. 
`define DDR4_PDEF(PNAME) \
  output wire        ``PNAME``_act_n, \
  output wire [17:0] ``PNAME``_adr, \
  input  wire        ``PNAME``_alert_n, \
